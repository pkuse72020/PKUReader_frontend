...
class _ReadNewsState extends State<ReadNews> {
  ...
  Widget mainImageWidget(height) => Container(
    ...
    decoration: BoxDecoration(
      image:DecorationImage(image: new ExactAssetImage(news_pic_dict[widget.title] ?? ""),fit:
      BoxFit.cover),
    ),
    child: Padding(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          IconButton(
            icon:(user.isPressFavButton)? Icon(Icons.bookmark_border,
              color: Colors.white,size: 28,):
            Icon(Icons.bookmark,color: Colors.white,size: 28,),
            onPressed: (){
              setState(() {
                if(user.isPressFavButton == true){
                  user.isPressFavButton = false;
                  if(!user.existArticle(Article(widget.title,
                      news_dict[widget.title]))) {
                    user.addArticle(Article(
                        widget.title, news_dict[widget.title]));
                  }
                }else{
                  user.isPressFavButton = true;
                  if(user.existArticle(Article(widget.title,
                      news_dict[widget.title]))) {
